...
namespace Gameiki.Patcher.Cecil {
    internal sealed class ChatModification : IModification {
        public void Execute(AssemblyDefinition assemblyDefinition) {
            ...
            // Terraria now supports chat within singleplayer mode
            var doUpdateHandleChat = main.Methods.FirstOrDefault(m => m.Name == "DoUpdate_HandleChat" && m.IsPrivate);
            var instructionIndex = doUpdateHandleChat.ScanInstructionPattern(OpCodes.Ldsfld, OpCodes.Ldstr, OpCodes.Call);
            var targetInstruction = doUpdateHandleChat.Body.Instructions[instructionIndex];
            doUpdateHandleChat.InsertBefore(targetInstruction,
                Instruction.Create(OpCodes.Ldsfld, main.Fields.FirstOrDefault(f => f.Name == "chatText")),
                Instruction.Create(OpCodes.Call,
                    main.Module.ImportReference(typeof(Hooks).GetMethod(nameof(Hooks.InvokeChat),
                        BindingFlags.NonPublic | BindingFlags.Static))),
                Instruction.Create(OpCodes.Brtrue_S, targetInstruction.Previous.Operand as Instruction));
            doUpdateHandleChat.ReplaceShortBranches();
        }
    }
}